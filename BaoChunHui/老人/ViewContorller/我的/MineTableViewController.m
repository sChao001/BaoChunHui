        ...
        case 1:
            switch (indexPath.row) {
                ...
                case 1:
                    // 积分
                {
                    scoreShopViewController *vc = [[scoreShopViewController alloc] initWithNibName:@"scoreShopViewController" bundle:nil];
                    [self.navigationController pushViewController:vc animated:YES];
                }
                    break;
                case 2:
...
- (void)CallTeleNum{
    ...
    UIAlertAction *actFamily = [UIAlertAction actionWithTitle:@"家属" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        NSString *tele = [userModel defaultsUserModel].teleNum;
        if (!tele) {// 判断家属电话是否存在
            // 弹出提示框，不存在号码
